library(Seurat); library(dplyr); library(ggplot2); library(patchwork);library(harmony)
...
object <- subset(object, subset = nCount_RNA < 13000)
object <- subset(object, subset = nFeature_RNA > 270 & nFeature_RNA < 3700)
...
object <- RunUMAP(object, dims = 1:50, umap.method = "uwot",reduction = "harmony")
object <- RunTSNE(object, dims = seq(50),reduction = "harmony")

## Find clusters
object <- FindNeighbors(object, reduction = "harmony", dims = seq(50),force.recalc = TRUE)
...
## calculate cluster expression
data <- GetAssayData(object, assay = "RNA", slot = "data")
data <- expm1(data)
Idents(object) <- "seurat_clusters"
mean_exp <- do.call(cbind, by(colnames(object), Idents(object),function(x) Matrix::rowMeans(data[, x,drop = F]), simplify = FALSE))
